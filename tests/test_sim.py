...
def _unpack_nibbles(payload: bytes) -> list[int]:
    """Return the per-pixel palette indices (0..15) from a packed chunk."""
    out: list[int] = []
    for b in payload:
        out.append(b & 0x0F)
        out.append(b >> 4)
...
def test_chunk_size(default_mandelbrot):
    """A rendered chunk is always exactly CHUNK_BYTES."""
    payload = render_chunk(default_mandelbrot, 0)
...
def test_palette_indices_in_range(default_mandelbrot):
    ...
    payload = render_chunk(default_mandelbrot, 5)
...
def test_mandelbrot_centre_chunk_has_variety(default_mandelbrot):
...
def test_mandelbrot_corner_chunk_mostly_escaped(default_mandelbrot):
    """Chunk 0 is the top-left corner. At zoom 0 with pan=(-0.5,0), this
    ...
    assert in_set < 1000, (
        f"corner chunk has {in_set} in-set pixels — expected near zero "
...
def test_chunk_id_out_of_range_rejected_clientside(default_mandelbrot):
    """chunk_id < 0 or > 15 raises ValueError before reaching the sim."""
    with pytest.raises(ValueError, match="chunk_id must be 0..15"):
        render_chunk(default_mandelbrot, 99)
...
def test_same_config_yields_same_bytes(default_mandelbrot):
    a = render_chunk(default_mandelbrot, 5)
    b = render_chunk(default_mandelbrot, 5)
    assert a == b
